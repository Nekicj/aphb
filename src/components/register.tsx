...
const inputCls =
  "w-full h-11 border border-neutral-200 rounded-lg px-3 text-sm bg-white text-neutral-900 outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all placeholder:text-neutral-400"
const labelCls =
  "block text-xs font-medium text-neutral-500 mb-1.5 uppercase tracking-wide"
const sectionTitle =
  "text-xs font-semibold text-neutral-400 uppercase tracking-widest mb-4 pb-2 border-b border-neutral-100"
const cardCls =
  "w-full rounded-2xl border border-neutral-200 bg-white shadow-lg p-6 md:p-8"
...
const MemberBlock = ({
  ...
  return (
    <div className="border border-neutral-100 rounded-xl p-5 space-y-4 bg-white">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-neutral-800">{title}</span>
        {badge && (
          <span className="text-xs bg-neutral-100 text-neutral-500 rounded-full px-2.5 py-0.5 font-medium">
            {badge}
          </span>
        )}
        {extra}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className={labelCls}>
            ФИО {!optional && <span className="text-red-400">*</span>}
          </label>
          <input
            name={`${prefix}Name`}
            type="text"
        ...
        <div>
          <label className={labelCls}>
            Школа {!optional && <span className="text-red-400">*</span>}
          </label>
          <input
            name={`${prefix}School`}
            type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div>
          <label className={labelCls}>
            Email {!optional && <span className="text-red-400">*</span>}
          </label>
          <input
            name={`${prefix}Email`}
            type="email"
        ...
        <div>
          <label className={labelCls}>
            Телефон {!optional && <span className="text-red-400">*</span>}
          </label>
          <input
            name={`${prefix}Phone`}
            type="text"
...
export function RegisterForm({ lang }: { lang: string }) {
  ...
  if (isSubmitted) {
    return (
      <div className={`${cardCls} bg-amber-50 border-amber-200 text-neutral-800 space-y-4`}>
        {lang === "kz" ? (
          <>
            <h3 className="text-xl font-semibold text-amber-800">
              Команда тіркелді, бірақ бұл әлі бәрі емес!
            </h3>
            <p className="text-sm text-neutral-700">
              Тіркеуді толық аяқтау үшін әрбір қатысушының ата-анасы немесе
              заңды өкілі AutoProctor жүйесін пайдалануға міндетті түрде келісім
              беруі керек.
            </p>
            <p className="text-sm text-neutral-500">
              Ата-анаңызға мына сілтемені жіберіңіз және олар пішінде сіздің
              телефон нөміріңізді немесе email-ыңызды көрсетуі керек екенін
              ескертіңіз:
            </p>
            <a
              href={parentFormUrl}
          ...
          <>
            <h3 className="text-xl font-semibold text-amber-800">
              Team registered — one more step!
            </h3>
            <p className="text-sm text-neutral-700">
              To fully complete registration, the parent or legal representative
              of each participant must submit a separate consent form for
              AutoProctor.
            </p>
            <p className="text-sm text-neutral-500">
              Send this link to your parents and remind them to enter your phone
              number or email so we can link the records:
            </p>
            <a
              href={parentFormUrl}
              className="inline-block bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2.5 px-5 rounded-lg transition-colors text-sm"
          ...
          <>
            <h3 className="text-xl font-semibold text-amber-800">
              Команда зарегистрирована — осталось одно действие!
            </h3>
            <p className="text-sm text-neutral-700">
              Чтобы завершить регистрацию, родитель/опекун или доверенное лицо
              каждого участника должен заполнить отдельную форму согласия на
              AutoProctor.
            </p>
            <p className="text-sm text-neutral-500">
              Перешлите ссылку ниже и напомните указать ваш номер телефона или
              email — так мы свяжем анкеты в базе:
            </p>
            <a
              href={parentFormUrl}
              className="inline-block bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2.5 px-5 rounded-lg transition-colors text-sm"
  ...
  return (
    <form
      ...
      className={`${cardCls} space-y-8 text-neutral-800`}
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <label className={labelCls}>
              Лига <span className="text-red-400">*</span>
            </label>
            <select
              name="league"
              value={league}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className={labelCls}>
              Язык участия <span className="text-red-400">*</span>
            </label>
            ...
            <p className="text-xs text-neutral-400 mt-1.5 leading-relaxed">
              Все участники команды должны владеть казахским, русским или обоими
              языками.
            </p>
          </div>
        </div>
      ...
      <div className="bg-neutral-50 border border-neutral-100 rounded-xl p-5 space-y-4">
        ...
        <p className="text-xs text-neutral-600 leading-relaxed bg-white p-3 rounded-lg border border-neutral-200">
          Руководитель команды — лицо, ответственное за команду. Не может быть
          участником команды или несовершеннолетним. Должен быть
          совершеннолетним с нотариально заверенной доверенностью от родителей
          несовершеннолетних участников. Может быть преподавателем, сотрудником
          школы или любым уполномоченным взрослым. Обязателен для всех команд.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className={labelCls}>
              Страна <span className="text-red-400">*</span>
            </label>
            <input
              name="leaderCountry"
              type="text"
      ...
      <div className="space-y-3">
        ...
        {!showMember4 ? (
          <button
            ...
            className="w-full border border-dashed border-neutral-200 rounded-xl py-3 text-sm text-neutral-400 hover:border-neutral-300 hover:text-neutral-600 transition-colors flex items-center justify-center gap-1.5 bg-transparent"
          >
          ...
          <MemberBlock
            ...
            extra={
              <button
                ...
                className="ml-auto text-xs text-red-400 hover:text-red-600 transition-colors bg-transparent"
              >
                Убрать
              </button>
            }
      ...
      <label className="flex items-start gap-3 cursor-pointer p-4 bg-neutral-50 rounded-xl border border-neutral-100">
        ...
        />
        <span className="text-xs text-neutral-500 leading-relaxed">
          Даю согласие на сбор, обработку и хранение Персональных Данных в
          соответствии с Перечнем №03-08/05. С{" "}
          <a
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full h-12 bg-[#172967] hover:bg-[#0f1c4a] text-white rounded-xl text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
